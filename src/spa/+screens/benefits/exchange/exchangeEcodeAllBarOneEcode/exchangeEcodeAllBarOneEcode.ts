...
@IonicPage()
@Component({
  selector: 'page-exchangeEcodeAllBarOneEcode',
  templateUrl: 'exchangeEcodeAllBarOneEcode.html',
  providers: [
    ExchangeEcodeAllBarOneEcodeService
  ]
})
export class ExchangeEcodeAllBarOneEcodePage {
  invalidValue: boolean;
  maxQuantityError: boolean;
  otherQuantityError: boolean;
  minQuartryError: boolean;
  anyValues: any[];
  otherValueError: boolean;
  minValueErr: boolean;
  maxValueErr: boolean;
  quantity: any;
  otherQuantity: any;
  ...
  constructor(public routeManager: RouteManagerService,
              ...
              public exchangeEcodeAllBarOneEcodeService: ExchangeEcodeAllBarOneEcodeService,
  ...
  get allOutOfStock() {
    return !(this.ecode && this.ecode.products && this.ecode.products.find((p) => p.stockAvailable));
  }
  ...
  selectCardValue(productId) {
    this.selectedProduct = this.ecode.products.find(p => p.productId == productId);
    if (this.selectedProduct) {
        this.selectedProductId = this.selectedProduct.productId;
    }
  ...
  private _buildProductsValueOption() {
    return this.ecode.products.map((product, index) => {
      return {
        id: index,
        text: product.name,
  ...
  private _selectDefaultProduct() {
    if (this.anyType) {
      ...
      return this.ecode && this.ecode.products && this.ecode.products.find(p => p.default);
    }
  }
